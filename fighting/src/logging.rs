...
use crate::{cmd::Cli, config::GlobalConfig};
use time::{format_description, UtcOffset};
use tracing_subscriber::fmt::time::OffsetTime;
use tracing_subscriber::layer::SubscriberExt;
use tracing_subscriber::util::SubscriberInitExt;
use tracing_subscriber::Layer;
use tracing_subscriber::{filter::dynamic_filter_fn, EnvFilter};

pub async fn init_logging(config: Option<&GlobalConfig>, cli: &Cli) {
    if std::env::var("RUST_LOG").is_err() {
        match cli.debug {
            0 => std::env::set_var("RUST_LOG", "fighting=info"),
            1 => std::env::set_var("RUST_LOG", "fighting=debug"),
            2 => std::env::set_var("RUST_LOG", "fighting=debug,russh=debug"),
